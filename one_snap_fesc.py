...
fesc_flat = fesc.flatten()

# Repeat each halo's properties 10 times to match the 10 fesc directions
lum_repeated = np.repeat(lum, 10)
lum1500_repeated = np.repeat(lum1500, 10)
minit_repeated = np.repeat(minit, 10)
mstar_repeated = np.repeat(mstar, 10)
mvir_repeated = np.repeat(mvir, 10)
sfr1_repeated = np.repeat(sfr1, 10)
sfr10_repeated = np.repeat(sfr10, 10)
sfr100_repeated = np.repeat(sfr100, 10)
sfr3_repeated = np.repeat(sfr3, 10)
stellar_Zlw_repeated = np.repeat(stellar_Zlw, 10)
stellar_Zmw_repeated = np.repeat(stellar_Zmw, 10)
stellar_age_repeated = np.repeat(stellar_age, 10)
...
ax1 = axes[0, 0]
ax1.scatter(Llambda_to_M1500(lum_repeated[lum_repeated>0]), fesc_flat[lum_repeated>0], alpha=0.3, s=20, label='lum (sim)', color='blue')
ax1.scatter(Llambda_to_M1500(lum1500_repeated[lum1500_repeated>0]), fesc_flat[lum1500_repeated>0], alpha=0.3, s=20, label='lum1500 (sim)', color='cyan')
...
# Subplot 3: M1500 
ax2 = axes[0, 1]
ax2.scatter(mstar_repeated, fesc_flat, alpha=0.3, s=20, label='mstar (sim)', color='red')
ax2.scatter(mvir_repeated, fesc_flat, alpha=0.3, s=20, label='mvir (sim)', color='orange')
ax2.scatter(minit_repeated, fesc_flat, alpha=0.3, s=20, label='minit (sim)', color='pink')

ax2.scatter(obs_mstar, obs_fesc, alpha=0.8, s=100, label='mstar (obs)', 
           color='darkred', marker='*', edgecolors='black', linewidths=1)
ax2.set_xlabel('Mass')
ax2.set_ylabel('LyC_fesc_dir')
ax2.set_xscale('log')
...
ax3 = axes[1, 0]
ax3.scatter(stellar_Zlw_repeated, fesc_flat, alpha=0.3, s=20, 
           label='stellar_Zlw (sim)', color='green')
ax3.scatter(stellar_Zmw_repeated, fesc_flat, alpha=0.3, s=20, 
...
ax4 = axes[1, 1]
ax4.scatter(sfr1_repeated, fesc_flat, alpha=0.3, s=20, label='sfr1 (sim)', color='purple')
ax4.scatter(sfr3_repeated, fesc_flat, alpha=0.3, s=20, label='sfr3 (sim)', color='violet')
ax4.scatter(sfr10_repeated, fesc_flat, alpha=0.3, s=20, label='sfr10 (sim)', color='magenta')
ax4.scatter(sfr100_repeated, fesc_flat, alpha=0.3, s=20, label='sfr100 (sim)', color='orchid')
...
plt.tight_layout()
plt.savefig(f'fesc_analysis_with_obs_{snapshot}.png', dpi=300, bbox_inches='tight')
logging.info(f"\nPlot saved as: fesc_analysis_with_obs_{snapshot}.png")
plt.show()
...
logging.info(f"  Total number of fesc values (halos × 10 directions): {len(fesc_flat)}")
logging.info(f"  LyC_fesc_dir range: [{np.min(fesc_flat):.4f}, {np.max(fesc_flat):.4f}]")
logging.info(f"  LyC_fesc_dir mean: {np.mean(fesc_flat):.4f}")
logging.info(f"  LyC_fesc_dir median: {np.median(fesc_flat):.4f}")
